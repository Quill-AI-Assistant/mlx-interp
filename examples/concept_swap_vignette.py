#!/usr/bin/env python3
"""
Concept swap proof of concept: swap green↔red at the activation level.
...
class SwapLayer(nn.Module):
    """Mirrors activations across the concept plane."""
...
def extract_concept_direction(model, tokenizer):
...
def run_test(model, tokenizer, directions, alpha, include_control=True):
    """Run concept swap tests with random-direction control."""
    test_questions = [
...
def main():
    parser = argparse.ArgumentParser(description="Concept swap PoC: swap green↔red")
    ...
    directions = extract_concept_direction(model, tokenizer)
    ...
    # Interactive
    print(f"\n{'='*60}")
    print(f"CONCEPT SWAP PoC | Layers: {sorted(directions.keys())}")
    print(f"{'='*60}")
    print(f"  /swap     - Toggle concept swap")
    print(f"  /alpha N  - Set strength (default {args.alpha})")
    print(f"  /test     - Run concept tests")
    print(f"  Ctrl+C    - Quit\n")
